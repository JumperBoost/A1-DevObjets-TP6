...
9. **Bonus** : Réfléchissez à une autre implémentation du programme de Sudoku utilisant une autre méthode `genererFils()` : l'ensemble des grilles filles d'une configuration donnée correspondent à l'ensemble des grilles valides contenant une unique et même case remplie avec l'une des valeurs valides (cette case peut être choisie arbitrairement). Pour le Sudoku de taille $`9`\times 9`$ par exemple, le nombre de grilles filles générées à chaque itération est donc dans ce cas au maximum 9. Observez alors que le test d'appartenance d'une nouvelle fille à l'ensemble `dejaVus` n'est pas utile. On peut en effet démontrer par récurrence que les grilles générées sont toutes différentes entre elles. Comment modifier votre programme pour pouvoir prendre en compte ce type de jeu ?
